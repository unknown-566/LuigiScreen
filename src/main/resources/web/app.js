(() => {
  ...
  const routes = {
    dashboard: ["OVERVIEW", "Dashboard", "Live operational overview of every screen.", "Ĺ˝ivĂ˝ provoznĂ­ pĹ™ehled vĹˇech plĂˇten."],
    screens: ["BUILD", "Screens", "Physical displays, playback state and rendering health.", "FyzickĂˇ plĂˇtna, stav pĹ™ehrĂˇvĂˇnĂ­ a vykreslovĂˇnĂ­."],
    media: ["BUILD", "Media Library", "Every local asset and its production readiness.", "VĹˇechna lokĂˇlnĂ­ mĂ©dia a jejich pĹ™ipravenost."],
    playlists: ["BUILD", "Playlist Editor", "Sequence and smart rotation without blind random choices.", "Sekvence a chytrĂˇ rotace bez nĂˇhodnĂ©ho hĂˇdĂˇnĂ­."],
    events: ["BUILD", "Events", "Build and inspect controlled broadcast timelines.", "Tvorba a kontrola Ĺ™Ă­zenĂ˝ch vysĂ­lacĂ­ch ÄŤasovĂ˝ch os."],
    automations: ["BUILD", "Automations", "Readable triggers, conditions and resulting actions.", "ÄŚitelnĂ© spouĹˇtÄ›ÄŤe, podmĂ­nky a vĂ˝slednĂ© akce."],
    live: ["OPERATE", "Live Studio", "Prepare privately, then take the source live.", "PĹ™iprav zdroj v nĂˇhledu a potom ho poĹˇli ĹľivÄ›."],
    schedule: ["OPERATE", "Automation Calendar", "Server-time automation rules and conflict detection.", "ÄŚasovĂˇ pravidla automatizace a kontrola konfliktĹŻ."],
    monitoring: ["SYSTEM", "Monitoring", "Real-time load, frame flow and per-screen performance.", "ZĂˇtÄ›Ĺľ, tok snĂ­mkĹŻ a vĂ˝kon jednotlivĂ˝ch plĂˇten."],
    diagnostics: ["SYSTEM", "Diagnostics", "Actionable checks and explanations, not raw errors.", "SrozumitelnĂ© kontroly a vysvÄ›tlenĂ­ mĂ­sto holĂ˝ch chyb."],
    configuration: ["SYSTEM", "Configuration", "Stage safe changes before publishing them.", "PĹ™iprav bezpeÄŤnĂ© zmÄ›ny pĹ™ed jejich publikovĂˇnĂ­m."],
    settings: ["SYSTEM", "Settings", "Studio access, session and server information.", "PĹ™Ă­stup ke Studiu, relace a informace o serveru."]
  };
  const czechTitles = { dashboard:"PĹ™ehled", screens:"PlĂˇtna", media:"Knihovna mĂ©diĂ­",
    playlists:"Editor playlistĹŻ", events:"Eventy", automations:"Automatizace",
    live:"Ĺ˝ivĂ© studio", schedule:"KalendĂˇĹ™ automatizacĂ­", monitoring:"Monitoring",
    diagnostics:"Diagnostika", configuration:"Konfigurace", settings:"NastavenĂ­" };
  ...
  async function load() {
    try {
      ...
      $("#workspaceContent").innerHTML = `<div class="empty"><div><h2>${esc(tx("Studio could not connect", "Studio se nemĹŻĹľe pĹ™ipojit"))}</h2><p>${esc(error.message)}</p></div></div>`;
    }
  }
    ...
    source.addEventListener("state", event => {
      try {
        ...
        if ((!active || !["INPUT", "SELECT", "TEXTAREA"].includes(active.tagName))
          && Date.now() >= app.deferRenderUntil) render(false);
  ...
  function updateChrome() {
    ...
    $("#draftState").textContent = external ? tx("External config change", "ExternĂ­ zmÄ›na configu") : dirty
      ? tx(`${app.state.session.draftChanges} unpublished`, `${app.state.session.draftChanges} nepublikovĂˇno`)
      : tx("No changes", "Ĺ˝ĂˇdnĂ© zmÄ›ny");
  ...
  function updateNavigationLabels() {
    const labels = {
      dashboard: ["Dashboard", "PĹ™ehled"], screens: ["Screens", "PlĂˇtna"], media: ["Media", "MĂ©dia"],
      playlists: ["Playlists", "Playlisty"], events: ["Events", "Eventy"], automations: ["Automations", "Automatizace"],
      live: ["Live Studio", "Ĺ˝ivĂ© studio"], schedule: ["Calendar", "KalendĂˇĹ™"], monitoring: ["Monitoring", "Monitoring"],
      diagnostics: ["Diagnostics", "Diagnostika"], configuration: ["Configuration", "Konfigurace"], settings: ["Settings", "NastavenĂ­"]
    };
  ...
  function renderDashboard() {
    ...
    return `${renderLaunchpad()}<div class="metrics">
      ${metric(tx("Screens online", "PlĂˇtna online"), `${s.activeScreens}/${s.totalScreens}`, tx("configured displays", "nastavenĂ˝ch plĂˇten"), "Enabled screens compared with every configured display.", "PoÄŤet zapnutĂ˝ch plĂˇten oproti vĹˇem nastavenĂ˝m.", s.activeScreens ? "good" : "warn")}
      ${metric(tx("Current viewers", "AktuĂˇlnĂ­ divĂˇci"), s.viewers, tx("receiving maps", "pĹ™ijĂ­mĂˇ mapy"), "Unique screen receiver count sampled by the plugin.", "PoÄŤet pĹ™Ă­jemcĹŻ plĂˇten mÄ›Ĺ™enĂ˝ pluginem.")}
      ${metric(tx("Active broadcasts", "AktivnĂ­ vysĂ­lĂˇnĂ­"), s.broadcasts, tx("live controllers", "ĹľivĂ˝ch Ĺ™adiÄŤĹŻ"), "Screens with a running source, playlist or event controller.", "PlĂˇtna s bÄ›ĹľĂ­cĂ­m zdrojem, playlistem nebo eventem.")}
      ${metric(tx("System health", "Stav systĂ©mu"), healthLabel(s.health), `${s.warnings} ${tx("alerts", "upozornÄ›nĂ­")}`, "Combined health from sources, media, playlists, MapEngine and server timing.", "SouhrnnĂ˝ stav zdrojĹŻ, mĂ©diĂ­, playlistĹŻ, MapEngine a serveru.", s.health === "healthy" ? "good" : s.health === "critical" ? "bad" : "warn")}
    </div>
    <div class="grid-two"><section class="panel">${panelHead(tx("On air", "PrĂˇvÄ› vysĂ­lĂˇ"), "Screens currently visible to players.", "PlĂˇtna, kterĂˇ nynĂ­ mohou vidÄ›t hrĂˇÄŤi.", `<small>${onAir.length}</small>`)}
      ${onAir.length ? `<div class="screen-grid">${onAir.map(screenCard).join("")}</div>` : empty("No active screens.", "Ĺ˝ĂˇdnĂˇ aktivnĂ­ plĂˇtna.")}
    </section><div><section class="panel">${panelHead(tx("Upcoming", "NadchĂˇzejĂ­cĂ­"), "The nearest enabled schedule entries and queued work.", "NejbliĹľĹˇĂ­ aktivnĂ­ plĂˇny a pĹ™ipravenĂ˝ obsah.")}${renderUpcoming()}</section>
      <section class="panel">${panelHead(tx("Alerts", "UpozornÄ›nĂ­"), "Only issues that can affect playback or publishing are shown here.", "Zde se ukazujĂ­ jen problĂ©my ovlivĹujĂ­cĂ­ pĹ™ehrĂˇvĂˇnĂ­ nebo publikovĂˇnĂ­.")}${renderAlerts()}</section></div></div>`;
  }
  ...
  function renderLaunchpad() {
    ...
    const hosts = (web.lanHosts || []).slice(0, 2).join(", ") || tx("auto detected", "automaticky zjiĹˇtÄ›no");
    const step = (id, number, title, text, action, command = "") => `<article class="launch-step ${nextStep === id ? "active" : ""}">
      ...
      ${command ? `<code>${esc(command)}</code>` : ""}</div><button class="button ${nextStep === id ? "primary" : "ghost"}" data-jump="${esc(action)}">${esc(tx("Open", "OtevĹ™Ă­t"))}</button></article>`;
    return `<section class="launchpad">
      <div class="launch-copy"><span class="eyebrow">${esc(tx("START HERE", "ZAÄŚNI TADY"))}</span>
        <h2>${esc(tx("No config hunting. Build the screen from here.", "Ĺ˝ĂˇdnĂ˝ lov v configu. PlĂˇtno postavĂ­Ĺˇ odsud."))}</h2>
        <p>${esc(tx("Use one web link, drop media into the media folder, then control what players see.", "PouĹľij jeden web odkaz, nahĂˇzej mĂ©dia do sloĹľky a pak Ĺ™Ă­dĂ­Ĺˇ, co hrĂˇÄŤi vidĂ­."))}</p>
        <div class="launch-badges"><span>${esc(web.lanReady ? tx("LAN ready", "LAN pĹ™ipraveno") : tx("Local only", "Jen lokĂˇlnÄ›"))}</span><span>Port ${esc(web.port || 8765)}</span><span>${esc(hosts)}</span></div>
      </div>
      <div class="launch-steps">
        ${step("screen", "01", tx("Place the canvas", "Postav plĂˇtno"), tx("Stand in Minecraft, look at a wall and create the map display.", "Stoupni si ve hĹ™e, koukni na zeÄŹ a vytvoĹ™ mapovĂ© plĂˇtno."), "screens", "/screen create main 7 4")}
        ${step("media", "02", tx("Add media", "PĹ™idej mĂ©dia"), tx("Put images, GIFs or videos into the LuigiScreen media folder.", "Dej obrĂˇzky, GIFy nebo videa do media sloĹľky LuigiScreen."), "media", app.state.config.mediaDirectory)}
        ${step("playlist", "03", tx("Make rotation", "UdÄ›lej rotaci"), tx("Create a playlist so the screen can run without babysitting.", "VytvoĹ™ playlist, aby plĂˇtno bÄ›Ĺľelo samo bez hlĂ­dĂˇnĂ­."), "playlists")}
        ${step("live", "04", tx("Go live", "PusĹĄ to live"), tx("Preview a source, then take it live only when it looks right.", "NejdĹ™Ă­v nahledni zdroj a aĹľ pak ho poĹˇli hrĂˇÄŤĹŻm."), "live")}
      </div>
    </section>`;
  }
  ...
  function renderUpcoming() {
    ...
    app.state.screens.forEach(screen => screen.queue.slice(0, 2).forEach(item => items.push({ id: item, nextLabel: tx("Queued", "Ve frontÄ›"), action: "queue", target: screen.id })));
    if (!items.length) return empty("Nothing is scheduled or queued.", "Nic nenĂ­ naplĂˇnovanĂ© ani ve frontÄ›.");
    return `<div class="upcoming-list">${items.slice(0, 8).map(item => `<div class="upcoming-item"><span class="tag ${item.action === "event" ? "event" : ""}">${esc(item.action)}</span><div><strong>${esc(item.value || item.id)}</strong><small>${esc(item.target || "")} Â· ${esc(item.nextLabel || "")}</small></div>${info("This item is expected to take control next.", "Tato poloĹľka by mÄ›la pĹ™evzĂ­t Ĺ™Ă­zenĂ­ jako dalĹˇĂ­.")}</div>`).join("")}</div>`;
  }

  function renderAlerts() {
    if (!app.state.alerts.length) return empty("No actionable alerts.", "Ĺ˝ĂˇdnĂˇ upozornÄ›nĂ­ vyĹľadujĂ­cĂ­ zĂˇsah.");
  ...
  function healthLabel(value) {
    return ({ healthy: tx("Healthy", "V poĹ™Ăˇdku"), warnings: tx("Warnings", "VarovĂˇnĂ­"), critical: tx("Critical", "KritickĂ©") })[value] || value;
  }
  ...
  function screenCard(screen) {
    ...
    return `<article class="screen-card ${stateClass} ${app.selected?.type === "screen" && app.selected.id === screen.id ? "selected" : ""}" data-screen="${esc(screen.id)}">
      ...
      <div class="card-body"><div class="card-title"><h3>${esc(screen.id)} ${info("Open this screen's live controls, automation, location, performance and history.", "OtevĹ™e ĹľivĂ© ovlĂˇdĂˇnĂ­, automatizaci, umĂ­stÄ›nĂ­, vĂ˝kon a historii plĂˇtna.")}</h3><span class="status ${esc(screen.state)}">${esc(screen.state)}</span></div>
      <div class="card-meta"><span>${tx("Now playing", "NynĂ­ hraje")}<b>${esc(screen.current)}</b></span><span>${tx("Viewers", "DivĂˇci")}<b>${screen.viewers}</b></span><span>${tx("Output", "VĂ˝stup")}<b>${screen.width}x${screen.height} maps</b></span><span>${tx("Actual FPS", "SkuteÄŤnĂ© FPS")}<b>${fmt(screen.actualFps)}</b></span></div></div></article>`;
  }
  ...
  function renderScreens() {
    ...
    const toolbar = `<div class="toolbar"><div class="view-switch">${layouts.map(([id,label]) => `<button data-layout="${id}" class="${app.screenLayout === id ? "active" : ""}">${label}</button>`).join("")}</div>${info("Grid is visual, List is compact, and World groups displays by dimension.", "MĹ™Ă­Ĺľka je vizuĂˇlnĂ­, seznam ĂşspornĂ˝ a svÄ›ty seskupujĂ­ plĂˇtna podle dimenze.")}</div>`;
    if (!app.state.screens.length) return toolbar + empty("Create a screen in Minecraft with /screen create.", "VytvoĹ™ plĂˇtno v Minecraftu pĹ™Ă­kazem /screen create.");
    ...
    if (app.screenLayout === "world") {
      ...
      return toolbar + Object.entries(worlds).map(([world, screens]) => `<section class="panel">${panelHead(world, "Screens physically placed in this Minecraft world.", "PlĂˇtna fyzicky umĂ­stÄ›nĂˇ v tomto svÄ›tÄ› Minecraftu.", `<small>${screens.length}</small>`)}<div class="screen-grid">${screens.map(screenCard).join("")}</div></section>`).join("");
    }
    return toolbar + `<div class="screen-grid">${app.state.screens.map(screenCard).join("")}</div>`;
  }

  function renderScreenDetail(screen) {
    const tabs = [["overview", tx("Overview", "PĹ™ehled")], ["automation", tx("Automation", "Automatika")], ["location", tx("Location", "Lokace")], ["performance", tx("Performance", "VĂ˝kon")], ["history", tx("History", "Historie")]];
    if (!tabs.some(([id]) => id === app.screenDetailTab)) app.screenDetailTab = "overview";
    const active = app.screenDetailTab;
    return `<div class="toolbar"><button class="button ghost" data-back-screens>${tx("Back to screens", "ZpÄ›t na plĂˇtna")}</button><span class="tag ${screen.mode === "event" ? "event" : "live"}">${esc(screen.mode)}</span><span class="tag">${screen.playlist ? esc(screen.playlist) : tx("direct source", "pĹ™Ă­mĂ˝ zdroj")}</span></div>
      <section class="panel"><div class="panel-head"><div><span class="eyebrow">SCREEN</span><h2>${esc(screen.id)} ${info("The selected physical map display and its active controller.", "VybranĂ© fyzickĂ© mapovĂ© plĂˇtno a jeho aktivnĂ­ Ĺ™adiÄŤ.")}</h2></div><small>${screen.viewers} ${tx("viewers", "divĂˇkĹŻ")}</small></div>
      <div class="grid-two"><div><div class="preview" style="aspect-ratio:${screen.width}/${screen.height}">${screen.preview ? `<img src="${esc(screen.preview)}" alt="">` : `<span class="signal"></span>`}${screen.enabled ? `<span class="on-air">ON AIR</span>` : ""}</div><div class="button-row"><button class="button ${screen.enabled ? "danger" : "success"}" data-action="screen.${screen.enabled ? "stop" : "start"}" data-screen-id="${screen.id}">${screen.enabled ? tx("Stop screen", "Zastavit plĂˇtno") : tx("Start screen", "Zapnout plĂˇtno")}</button><button class="button ghost" data-action="${screen.paused ? "playback.resume" : "playback.pause"}" data-screen-id="${screen.id}">${screen.paused ? tx("Resume", "PokraÄŤovat") : tx("Pause", "Pozastavit")}</button><button class="button ghost" data-action="playback.skip" data-screen-id="${screen.id}">Skip</button><button class="button ghost" data-action="playback.repeat" data-screen-id="${screen.id}">Repeat</button><button class="button primary" data-action="playback.return" data-screen-id="${screen.id}">${tx("Return auto", "VrĂˇtit automatiku")}</button></div></div>
      <div><h3>${tx("Now playing", "NynĂ­ hraje")} ${info("The exact media label currently controlled by LuigiScreen.", "PĹ™esnĂ˝ nĂˇzev mĂ©dia prĂˇvÄ› Ĺ™Ă­zenĂ©ho LuigiScreenem.")}</h3><h2>${esc(screen.current)}</h2><p class="muted">${esc(screen.controller)} - ${duration(screen.remaining)} ${tx("remaining", "zbĂ˝vĂˇ")}</p><div class="inspector-section"><h3>${tx("Why is this playing?", "ProÄŤ se toto pĹ™ehrĂˇvĂˇ?")}</h3><p>${esc(screen.reason)}</p></div><div class="property"><span>${tx("Next", "DalĹˇĂ­")}</span><b>${esc(screen.next)}</b></div><div class="property"><span>${tx("Queue", "Fronta")}</span><b>${screen.queue.length}</b></div></div></div></section>
  ...
  function renderScreenDetailTab(screen, tab) {
    ...
    if (tab === "location") return `<div class="grid-three screen-tab-content">${metric("World", screen.world, `${screen.x}, ${screen.y}, ${screen.z}`, "Stored Minecraft world and block coordinate.", "UloĹľenĂ˝ svÄ›t a souĹ™adnice bloku.")}${metric("Facing", screen.facing, `${screen.width}x${screen.height} maps`, "Wall direction and physical map size.", "SmÄ›r stÄ›ny a fyzickĂˇ velikost map.")}${metric("View permission", screen.permissionRequired ? screen.viewPermission : tx("not required", "nenĂ­ potĹ™eba"), `${fmt(screen.distance)} blocks`, "Who can see this screen and from how far.", "Kdo vidĂ­ plĂˇtno a z jakĂ© vzdĂˇlenosti.")}</div>`;
    if (tab === "performance") return `<div class="grid-three screen-tab-content">${metric("Target FPS", fmt(screen.targetFps), "configured", "Requested screen frame rate.", "PoĹľadovanĂˇ snĂ­mkovĂˇ frekvence plĂˇtna.")}${metric("Actual FPS", fmt(screen.actualFps), `${screen.renderedFrames} rendered`, "Frames actually rendered after adaptive limits.", "SkuteÄŤnÄ› vykreslenĂ© snĂ­mky po limitech.")}${metric("Render cost", `${fmt(screen.renderMillis)} ms`, `${screen.droppedFrames} dropped`, "Average map conversion time and dropped frames.", "PrĹŻmÄ›rnĂ˝ pĹ™evod map a zahozenĂ© snĂ­mky.")}</div><table><thead><tr><th>RECEIVED</th><th>RENDERED</th><th>DROPPED</th><th>RECONNECTS</th><th>LAST FRAME</th></tr></thead><tbody><tr><td>${screen.receivedFrames}</td><td>${screen.renderedFrames}</td><td>${screen.droppedFrames}</td><td>${screen.reconnects}</td><td>${duration(screen.lastFrameAge)}</td></tr></tbody></table>`;
    if (tab === "history") return screen.history?.length ? `<div class="upcoming-list screen-tab-content">${screen.history.slice().reverse().map(item => `<div class="upcoming-item"><span class="tag">LOG</span><div><strong>${esc(item)}</strong><small>${esc(screen.id)}</small></div></div>`).join("")}</div>` : empty("No playback history yet.", "ZatĂ­m tu nenĂ­ historie pĹ™ehrĂˇvĂˇnĂ­.");
    return `<div class="grid-three screen-tab-content">${metric("Source", screen.sourceType, screen.source, "The active typed media source. Sensitive URL values are masked.", "AktivnĂ­ typovanĂ˝ zdroj. CitlivĂ© ÄŤĂˇsti URL jsou skrytĂ©.")}${metric("Automation", screen.playlist || tx("Direct", "PĹ™Ă­mo"), screen.mode, "Playlist or event currently controlling this screen.", "Playlist nebo event, kterĂ˝ teÄŹ Ĺ™Ă­dĂ­ plĂˇtno.")}${metric("Location", screen.world, `${screen.x}, ${screen.y}, ${screen.z}`, "Stored world and upper-left wall coordinate.", "UloĹľenĂ˝ svÄ›t a souĹ™adnice levĂ©ho hornĂ­ho rohu.")}</div>`;
  }
  ...
  function renderScreenAutomation(screen) {
    ...
    const playlistBody = playlists.length
      ...
      : `<p class="muted">${tx("No playlists exist yet. Create one first, then assign it here.", "Zatim neexistuje zadny playlist. NejdĹ™Ă­v ho vytvor a pak ho tady prirad.")}</p><button class="button primary" data-jump="playlists">${tx("Create playlist", "Vytvorit playlist")}</button>`;
  ...
  function playlistOptions(selected = "", includeEmpty = false) {
    const emptyOption = includeEmpty ? `<option value="">${tx("No playlist", "Ĺ˝ĂˇdnĂ˝ playlist")}</option>` : "";
  ...
  function probabilityBars(items) {
    if (!items.length) return empty("No usable items.", "Ĺ˝ĂˇdnĂ© pouĹľitelnĂ© poloĹľky.");
    return items.map(item => `<div class="property"><span>${esc(item.id)} ${info("Estimated share among currently eligible weighted items.", "OdhadovanĂ˝ podĂ­l mezi aktuĂˇlnÄ› zpĹŻsobilĂ˝mi vĂˇĹľenĂ˝mi poloĹľkami.")}</span><b>${fmt(item.probability)}%</b></div><div class="weight-bar"><span style="width:${Math.min(100,item.probability)}%"></span></div>`).join("");
  }
  ...
  function renderAutomations() {
    ...
    const draftNew = newAutomationDraft(firstTarget);
    const creator = canEdit ? `<section class="panel builder-hero automation-hero">${panelHead(tx("Build an automation", "Vytvor automatizaci"), "A rule says WHEN something should happen and THEN what LuigiScreen should do.", "Pravidlo rika KDY se ma neco stat a POTOM co ma LuigiScreen udelat.")}<div class="automation-create-grid"><input id="newScheduleName" placeholder="${tx("evening_show", "vecerni_show")}" value="${esc(draftNew.name)}"><input id="newScheduleTime" type="time" value="${esc(draftNew.time)}"><select id="newScheduleTarget">${targetOptions(draftNew.target)}</select><select id="newScheduleAction">${automationActionOptions(draftNew.action)}</select><select id="newScheduleValue">${automationValueOptions(draftNew.action, draftNew.value)}</select><button class="button primary" data-create-schedule>${tx("Create rule", "Vytvorit pravidlo")}</button></div></section>` : "";
    ...
    const form = automationDraft(schedule);
    const preview = {...schedule, enabled: form.enabled === "true", time: form.time, target: form.target, action: form.action, value: form.value, priority: form.priority, conflict: form.conflict, nextLabel: form.time};
    return `<div class="toolbar"><button class="button ghost" data-back-automations>${tx("All automations", "Vsechny automatizace")}</button><span class="tag">${esc(schedule.id)}</span><span class="status ${preview.enabled ? "running" : "empty"}">${preview.enabled ? tx("enabled", "zapnuto") : tx("disabled", "vypnuto")}</span></div><div class="playlist-workspace automation-workspace"><div class="playlist-builder"><section class="panel automation-rule-canvas">${panelHead(schedule.id, "This is the rule in plain language. Unsaved edits stay in this browser until Save rule is pressed.", "Toto je pravidlo normalni reci. Neulozene upravy zustanou v prohlizeci, dokud nestisknes Save rule.", `<span class="tag">${esc(preview.nextLabel)}</span>`)}${automationRuleVisual(preview)}${conflicts.length ? `<div class="alert warning"><span class="alert-marker"></span><div><strong>${tx("Conflict detected", "Nalezen konflikt")}</strong><small>${esc(conflicts.join(", "))}</small></div></div>` : ""}</section></div><aside class="playlist-side"><section class="panel playlist-add-panel automation-edit-panel">${panelHead(tx("Edit rule", "Upravit pravidlo"), "Save writes this automation directly. No YAML editing needed.", "Save ulozi automatizaci primo. Bez YAML editace.")}<div class="field"><label>${tx("Enabled", "Zapnuto")}</label><select id="automationEnabled"><option value="true" ${form.enabled === "true" ? "selected" : ""}>${tx("Enabled", "Zapnuto")}</option><option value="false" ${form.enabled === "false" ? "selected" : ""}>${tx("Disabled", "Vypnuto")}</option></select></div><div class="field"><label>WHEN</label><input id="automationTime" type="time" value="${esc(form.time)}"></div><div class="field"><label>${tx("Target", "Cil")}</label><select id="automationTarget">${targetOptions(form.target)}</select></div><div class="field"><label>THEN</label><select id="automationAction">${automationActionOptions(form.action)}</select></div><div class="field"><label>${tx("Value", "Hodnota")}</label><select id="automationValue">${automationValueOptions(form.action, form.value)}</select></div><div class="field-row"><div class="field"><label>${tx("Priority", "Priorita")}</label><input id="automationPriority" type="number" min="0" value="${esc(form.priority)}"></div><div class="field"><label>${tx("Conflict", "Konflikt")}</label><select id="automationConflict"><option value="priority" ${form.conflict === "priority" ? "selected" : ""}>priority</option><option value="allow" ${form.conflict === "allow" ? "selected" : ""}>allow</option></select></div></div><button class="button primary" data-save-automation="${esc(schedule.id)}">${tx("Save rule", "Ulozit pravidlo")}</button></section><section class="panel playlist-action-panel">${panelHead(tx("Operate", "Ovladat"), "Run now ignores the clock and executes this rule immediately.", "Run now ignoruje cas a spusti pravidlo hned.")}<div class="button-row"><button class="button success" data-run-automation="${esc(schedule.id)}">${tx("Run now", "Spustit hned")}</button><button class="button ghost" data-jump="schedule">${tx("Open calendar", "Otevrit kalendar")}</button></div></section><section class="panel playlist-action-panel">${panelHead(tx("Manage rule", "Sprava pravidla"), "Duplicate before experimenting, delete when the rule is no longer needed.", "Duplikuj pred experimentem, smaz kdyz pravidlo uz nepotrebujes.")}<div class="field"><label>${tx("Duplicate as", "Duplikovat jako")}</label><input id="duplicateAutomationName" placeholder="${esc(schedule.id)}_copy"></div><div class="button-row"><button class="button ghost" data-duplicate-automation="${esc(schedule.id)}">${tx("Duplicate", "Duplikovat")}</button><button class="button danger" data-delete-automation="${esc(schedule.id)}">${tx("Delete rule", "Smazat pravidlo")}</button></div></section></aside></div>${groupCreator}`;
  }
  ...
  function automationRuleVisual(schedule) {
    ...
    return `<div class="automation-rule-flow"><div class="condition"><span><b>WHEN</b><br>${esc(schedule.nextLabel || schedule.time)}</span></div><div class="condition"><span><b>IF</b><br>${esc((schedule.days || []).join(", "))}</span></div><div class="condition"><span><b>THEN</b><br>${esc(schedule.action + thenValue)} on ${esc(schedule.target)}</span></div></div><small class="muted">${tx("Conflict policy", "Reseni konfliktu")}: ${esc(schedule.conflict)} Â· ${tx("server time", "cas serveru")} ${esc(schedule.time)}</small>`;
  }
  ...
  function newAutomationDraft(fallbackTarget) {
    const draft = app.newAutomationDraft || {};
    const action = draft.action || "event";
    return {
      name: draft.name || "",
      time: draft.time || "20:00",
      target: draft.target || fallbackTarget || "",
      action,
      value: draft.value || ""
    };
  }

  function automationDraft(schedule) {
    const draft = app.automationDrafts[schedule.id] || {};
    const action = draft.action ?? schedule.action;
    return {
      enabled: String(draft.enabled ?? schedule.enabled),
      time: draft.time ?? schedule.time,
      target: draft.target ?? schedule.target,
      action,
      value: draft.value ?? schedule.value ?? "",
      priority: String(draft.priority ?? schedule.priority),
      conflict: draft.conflict ?? schedule.conflict ?? "priority"
    };
  }

  function updateNewAutomationDraft(patch) {
    app.newAutomationDraft = {...(app.newAutomationDraft || {}), ...patch};
    app.deferRenderUntil = Date.now() + 10000;
  }

  function updateAutomationDraft(id, patch) {
    app.automationDrafts[id] = {...(app.automationDrafts[id] || {}), ...patch};
    app.deferRenderUntil = Date.now() + 10000;
  }
  ...
  function renderLive() {
    ...
    return `<div class="toolbar"><label>${tx("Screen target", "CĂ­lovĂ© plĂˇtno")} ${info("Preview stays private until Take Live sends it to this screen or group.", "NĂˇhled zĹŻstane soukromĂ˝, dokud ho Take Live nepoĹˇle na plĂˇtno nebo skupinu.")}</label><select id="liveTarget">${targetOptions(app.liveTarget)}</select><button class="button primary" data-return-auto>${tx("Return to automation", "VrĂˇtit automatizaci")}</button></div>
      <div class="studio-deck"><section class="monitor preview-monitor"><div class="monitor-head"><span>PREVIEW ${info("Prepared source that players cannot see yet.", "PĹ™ipravenĂ˝ zdroj, kterĂ˝ hrĂˇÄŤi jeĹˇtÄ› nevidĂ­.")}</span><span>${media ? esc(media.type) : "EMPTY"}</span></div><div class="preview">${media?.thumbnail ? `<img src="${esc(media.thumbnail)}" alt="">` : `<span class="signal"></span>`}</div><div class="card-body"><h2>${esc(media?.name || tx("No source selected", "NenĂ­ vybranĂ˝ zdroj"))}</h2><select id="previewSource"><option value="">${tx("Choose media", "Vyber mĂ©dium")}</option>${app.state.media.filter(item=>item.valid).map(item=>`<option value="${esc(item.id)}" ${item.id===app.previewMedia?"selected":""}>${esc(item.id)}</option>`).join("")}</select></div></section>
      <section class="monitor program"><div class="monitor-head"><span>PROGRAM / ON AIR ${info("This is what players at the target screen currently receive.", "Toto prĂˇvÄ› pĹ™ijĂ­majĂ­ hrĂˇÄŤi u cĂ­lovĂ©ho plĂˇtna.")}</span><span>${target?.enabled ? "ON AIR" : "OFF"}</span></div><div class="preview">${target?.preview ? `<img src="${esc(target.preview)}" alt="">` : `<span class="signal"></span>`}${target?.enabled ? `<span class="on-air">ON AIR</span>`:""}</div><div class="card-body"><h2>${esc(target?.current || "--")}</h2><p class="muted">${esc(target?.reason || "")}</p></div></section></div>
      <div class="take-live"><button data-take-live ${!media || !target ? "disabled" : ""}>TAKE LIVE</button></div>
      <section class="panel">${panelHead(tx("Quick sources", "RychlĂ© zdroje"), "Frequently used media can be prepared with one click before going live.", "ÄŚasto pouĹľĂ­vanĂˇ mĂ©dia pĹ™ipravĂ­Ĺˇ jednĂ­m kliknutĂ­m pĹ™ed vysĂ­lĂˇnĂ­m.")}<div class="quick-sources">${app.state.media.filter(item=>item.valid).slice(0,8).map(item=>`<button class="quick-source" data-cue-media="${esc(item.id)}"><b>${esc(item.name)}</b><br><small>${esc(item.type)}</small></button>`).join("")}</div></section>`;
  }
  ...
  function renderMonitoring() {
    const s = app.state.server;
    return `<div class="metrics">${metric("TPS", fmt(s.tps), "target 20.0", "Server ticks per second sampled by Paper.", "PoÄŤet tickĹŻ serveru za sekundu mÄ›Ĺ™enĂ˝ Paperem.", s.tps>18?"good":"warn")}${metric("MSPT", `${fmt(s.mspt)} ms`, "tick cost", "Average tick processing time.", "PrĹŻmÄ›rnĂ˝ ÄŤas zpracovĂˇnĂ­ ticku.", s.mspt<40?"good":"warn")}${metric("Rendered FPS", fmt(s.effectiveFps), `${s.renderedFrames} frames`, "Current primary output frame rate.", "AktuĂˇlnĂ­ rychlost hlavnĂ­ho vĂ˝stupu.")}${metric("Heap", bytes(s.memoryUsed), `of ${bytes(s.memoryMax)}`, "Java heap used by the server process; native FFmpeg memory is separate.", "Java heap serveru; nativnĂ­ pamÄ›ĹĄ FFmpeg se poÄŤĂ­tĂˇ zvlĂˇĹˇĹĄ.")}</div>
      <div class="grid-two"><section class="panel">${panelHead(tx("Server timing", "ÄŚasovĂˇnĂ­ serveru"), "Rolling samples received by this browser while Studio is open.", "PrĹŻbÄ›ĹľnĂ© vzorky pĹ™ijatĂ© tĂ­mto prohlĂ­ĹľeÄŤem bÄ›hem otevĹ™enĂ©ho Studia.")}<div class="chart">${chartBars("tps",20)}</div></section><section class="panel">${panelHead(tx("Rendered output", "VykreslenĂ˝ vĂ˝stup"), "Effective FPS after adaptive throttling.", "EfektivnĂ­ FPS po adaptivnĂ­m omezenĂ­.")}<div class="chart">${chartBars("fps",20)}</div></section></div>
      <section class="panel">${panelHead(tx("Per-screen performance", "VĂ˝kon jednotlivĂ˝ch plĂˇten"), "Compare target FPS, actual FPS, map count, packet pressure and rendering cost.", "PorovnĂˇ cĂ­lovĂ© a skuteÄŤnĂ© FPS, poÄŤet map a cenu vykreslenĂ­.")}<table><thead><tr><th>SCREEN</th><th>TARGET</th><th>ACTUAL</th><th>MAPS</th><th>VIEWERS</th><th>DROPPED</th><th>RENDER</th><th>PROFILE</th></tr></thead><tbody>${app.state.screens.map(screen=>`<tr><td>${esc(screen.id)}</td><td>${fmt(screen.targetFps)}</td><td>${fmt(screen.actualFps)}</td><td>${screen.maps}</td><td>${screen.viewers}</td><td>${screen.droppedFrames}</td><td>${fmt(screen.renderMillis)} ms</td><td><span class="tag">${performanceProfile(screen)}</span></td></tr>`).join("")}</tbody></table></section>`;
  }
  ...
  function renderDiagnostics() {
    ...
    return `<div class="grid-two"><section class="panel">${panelHead(tx("System check", "Kontrola systĂ©mu"), "A concise readiness checklist for the complete playback path.", "StruÄŤnĂˇ kontrola pĹ™ipravenosti celĂ© pĹ™ehrĂˇvacĂ­ cesty.")}${checks.map(([ok,name,detail])=>`<div class="diagnostic-check"><span class="check-mark ${ok?"":"fail"}">${ok?"OK":"X"}</span><b>${esc(name)} ${info("This check updates live and points to the subsystem responsible for playback.", "Kontrola se mÄ›nĂ­ ĹľivÄ› a ukazuje ÄŤĂˇst systĂ©mu odpovÄ›dnou za pĹ™ehrĂˇvĂˇnĂ­.")}</b><small class="muted">${esc(detail)}</small></div>`).join("")}</section><section class="panel">${panelHead(tx("Why not playing?", "ProÄŤ se nepĹ™ehrĂˇvĂˇ?"), "Select a screen to inspect the latest decision, source error and queue state.", "Vyber plĂˇtno pro poslednĂ­ rozhodnutĂ­, chybu zdroje a stav fronty.")}<select id="diagnosticScreen">${screenOptions(app.selected?.type==="screen"?app.selected.id:app.liveTarget)}</select>${diagnosticReason()}</section></div><section class="panel">${panelHead(tx("Recent problems", "PoslednĂ­ problĂ©my"), "Errors are sanitized so stream passwords and keys never appear here.", "Chyby jsou oÄŤiĹˇtÄ›nĂ©, takĹľe se zde neukĂˇĹľou hesla ani stream key.")}${renderAlerts()}</section>`;
  }
  ...
  function diagnosticReason() {
    ...
    if(!screen)return empty("No screen selected.","NenĂ­ vybranĂ© plĂˇtno.");
    return `<div class="inspector-section"><h3>${esc(screen.id)}</h3><p>${esc(screen.reason)}</p><div class="property"><span>${tx("Source state", "Stav zdroje")}</span><b>${esc(screen.state)}</b></div><div class="property"><span>${tx("Source error", "Chyba zdroje")}</span><b>${esc(screen.error)}</b></div><div class="property"><span>${tx("Next eligible", "DalĹˇĂ­ zpĹŻsobilĂ©")}</span><b>${esc(screen.next)}</b></div></div>`;
  }

  function renderConfiguration() {
    const c=app.state.config;
    return `<div class="grid-two"><section class="panel">${panelHead(tx("Performance draft", "Koncept vĂ˝konu"), "Values remain private to this session until Publish is pressed.", "Hodnoty zĹŻstanou v tĂ©to relaci, dokud nestiskneĹˇ Publikovat.")}
      ${draftField("performance.max-map-updates-per-second", "integer", tx("Maximum map updates per second", "Maximum aktualizacĂ­ map za sekundu"), c.maxMapUpdates, "Caps map packet pressure across the plugin.", "Omezuje mnoĹľstvĂ­ mapovĂ˝ch packetĹŻ v celĂ©m pluginu.")}
      ${draftToggle("performance.adaptive-fps", tx("Adaptive FPS", "AdaptivnĂ­ FPS"), c.adaptiveFps, "Automatically lowers output FPS for expensive displays.", "Automaticky snĂ­ĹľĂ­ FPS u nĂˇroÄŤnĂ˝ch plĂˇten.")}
      ${draftToggle("performance.pause-rendering-without-viewers", tx("Pause without viewers", "Pozastavit bez divĂˇkĹŻ"), c.pauseWithoutViewers, "Stops decoding when no eligible player can see a source.", "ZastavĂ­ dekĂłdovĂˇnĂ­, pokud zdroj nevidĂ­ ĹľĂˇdnĂ˝ oprĂˇvnÄ›nĂ˝ hrĂˇÄŤ.")}
      </section><section class="panel">${panelHead(tx("Publishing pipeline", "PublikaÄŤnĂ­ proces"), "Studio snapshots config.yml, applies staged values and performs a safe plugin reload.", "Studio zazĂˇlohuje config.yml, pouĹľije koncept a bezpeÄŤnÄ› znovu naÄŤte plugin.")}${app.state.session.externalChanges?`<div class="alert critical"><span class="alert-marker"></span><div><strong>${tx("Configuration changed outside Studio", "Konfigurace se zmÄ›nila mimo Studio")}</strong><small>${tx("Publishing is locked to prevent overwriting manual edits.", "PublikovĂˇnĂ­ je uzamÄŤenĂ©, aby nepĹ™epsalo ruÄŤnĂ­ Ăşpravy.")}</small></div></div>`:""}<div class="diagnostic-check"><span class="check-mark">1</span><b>Draft</b><small>${app.state.session.draftChanges} changes</small></div><div class="diagnostic-check"><span class="check-mark">2</span><b>Backup</b><small>config history</small></div><div class="diagnostic-check"><span class="check-mark">3</span><b>Validate</b><small>typed values</small></div><div class="diagnostic-check"><span class="check-mark">4</span><b>Publish</b><small>safe reload</small></div><div class="button-row"><button class="button publish" data-publish ${!app.state.session.draftChanges||app.state.session.externalChanges?"disabled":""}>${tx("Publish draft", "Publikovat koncept")}</button><button class="button ghost" data-discard ${!app.state.session.draftChanges?"disabled":""}>${tx("Discard", "Zahodit")}</button></div></section></div>
      <section class="panel">${panelHead(tx("Per-screen settings", "NastavenĂ­ plĂˇten"), "Select a screen and stage FPS, viewer distance or permission changes in its inspector.", "Vyber plĂˇtno a pĹ™iprav zmÄ›nu FPS, vzdĂˇlenosti nebo oprĂˇvnÄ›nĂ­ v inspectoru.")}<div class="screen-grid">${app.state.screens.map(screenCard).join("")}</div></section>`;
  }

  function draftField(path,type,label,value,helpEn,helpCs){return `<div class="field"><label>${esc(label)} ${info(helpEn,helpCs)}</label><div class="field-row"><input data-draft-input="${esc(path)}" data-type="${type}" value="${esc(value)}"><button class="button ghost" data-stage="${esc(path)}">${tx("Stage", "PĹ™ipravit")}</button></div></div>`;}
  function draftToggle(path,label,value,helpEn,helpCs){return `<div class="field"><label>${esc(label)} ${info(helpEn,helpCs)}</label><div class="field-row"><select data-draft-input="${esc(path)}" data-type="boolean"><option value="true" ${value?"selected":""}>true</option><option value="false" ${!value?"selected":""}>false</option></select><button class="button ghost" data-stage="${esc(path)}">${tx("Stage", "PĹ™ipravit")}</button></div></div>`;}

  function renderSettings() {
    const s=app.state.server, session=app.state.session;
    return `<div class="grid-two"><section class="panel">${panelHead(tx("Web Studio access", "PĹ™Ă­stup k Web Studiu"), "The HTTP server is local-only by default and requires a one-time login link.", "HTTP server je ve vĂ˝chozĂ­m stavu pouze lokĂˇlnĂ­ a vyĹľaduje jednorĂˇzovĂ˝ odkaz.")}<div class="property"><span>${tx("Server", "Server")} ${info("The name shown by this LuigiScreen instance.", "NĂˇzev tĂ©to instance LuigiScreen.")}</span><b>${esc(s.name)}</b></div><div class="property"><span>${tx("Version", "Verze")} ${info("Installed plugin version.", "NainstalovanĂˇ verze pluginu.")}</span><b>${esc(s.version)}</b></div><div class="property"><span>${tx("Session user", "UĹľivatel relace")} ${info("The command sender who created this temporary login.", "UĹľivatel, kterĂ˝ vytvoĹ™il toto doÄŤasnĂ© pĹ™ihlĂˇĹˇenĂ­.")}</span><b>${esc(session.actor)}</b></div><div class="property"><span>${tx("Session expires", "Relace vyprĹˇĂ­")} ${info("Expired sessions are rejected automatically.", "Relace po vyprĹˇenĂ­ plugin automaticky odmĂ­tne.")}</span><b>${new Date(session.expiresAt).toLocaleString()}</b></div><div class="property"><span>${tx("Live connection", "Ĺ˝ivĂ© spojenĂ­")} ${info("Server-Sent Events keep operational data current without reloading the page.", "Server-Sent Events udrĹľujĂ­ provoznĂ­ data aktuĂˇlnĂ­ bez obnovovĂˇnĂ­ strĂˇnky.")}</span><b>${app.live?"connected":"reconnecting"}</b></div><div class="button-row"><a class="button ghost" href="/logout">${tx("End this session", "UkonÄŤit tuto relaci")}</a></div></section>
      <section class="panel">${panelHead(tx("Role capabilities", "OprĂˇvnÄ›nĂ­ role"), "Capabilities are copied from Bukkit permissions when the login link is created.", "OprĂˇvnÄ›nĂ­ se pĹ™evezmou z Bukkit permissions pĹ™i vytvoĹ™enĂ­ odkazu.")}<div class="quick-sources">${session.capabilities.map(item=>`<span class="tag live">${esc(item)}</span>`).join("")}</div><p class="muted" style="margin-top:14px">${tx("Create a new link after changing permissions. Existing sessions do not silently gain access.", "Po zmÄ›nÄ› oprĂˇvnÄ›nĂ­ vytvoĹ™ novĂ˝ odkaz. ExistujĂ­cĂ­ relace nezĂ­skajĂ­ pĹ™Ă­stup potajĂ­.")}</p></section></div>`;
  }

  function renderInspector() {
    const box=$("#inspectorContent"), title=$("#inspectorTitle");
    $("#app").classList.toggle("inspector-active", Boolean(app.selected));
    if(!app.selected){title.textContent=tx("Nothing selected","Nic nenĂ­ vybranĂ©");box.innerHTML=`<p class="muted">${tx("Select an object in the workspace to inspect it here.","Vyber objekt v pracovnĂ­ ploĹˇe a zde uvidĂ­Ĺˇ jeho detail.")}</p>`;return;}
  ...
  function inspectScreen(screen){if(!screen)return;$("#inspectorTitle").textContent=screen.id;$("#inspectorContent").innerHTML=`<section class="inspector-section"><h3>${tx("Status","Stav")} ${info("Current source and playback state.","AktuĂˇlnĂ­ stav zdroje a pĹ™ehrĂˇvĂˇnĂ­.")}</h3><div class="property"><span>State</span><b>${esc(screen.state)}</b></div><div class="property"><span>Content</span><b>${esc(screen.current)}</b></div><div class="property"><span>Controller</span><b>${esc(screen.controller)}</b></div><div class="property"><span>Error</span><b>${esc(screen.error)}</b></div></section><section class="inspector-section"><h3>${tx("Properties","Vlastnosti")} ${info("Staged values apply only after Publish.","PĹ™ipravenĂ© hodnoty se pouĹľijĂ­ aĹľ po publikovĂˇnĂ­.")}</h3>${draftField(`screens.${screen.id}.fps`,"double","Target FPS",screen.targetFps,"Requested frames per second from 0.1 to 20.","PoĹľadovanĂ© FPS od 0,1 do 20.")}${draftField(`screens.${screen.id}.distance`,"double",tx("Viewer distance","VzdĂˇlenost divĂˇkĹŻ"),screen.distance,"Maximum range for map receivers.","MaximĂˇlnĂ­ vzdĂˇlenost pĹ™Ă­jemcĹŻ map.")}${draftToggle(`screens.${screen.id}.enabled`,tx("Enabled","Zapnuto"),screen.enabled,"Whether the display starts and accepts media.","Zda se plĂˇtno spouĹˇtĂ­ a pĹ™ijĂ­mĂˇ mĂ©dia.")}${draftToggle(`screens.${screen.id}.permission-required`,tx("Require view permission","VyĹľadovat oprĂˇvnÄ›nĂ­"),screen.permissionRequired,"When enabled, players need the per-screen see permission.","Po zapnutĂ­ hrĂˇÄŤi potĹ™ebujĂ­ oprĂˇvnÄ›nĂ­ see pro toto plĂˇtno.")}</section><section class="inspector-section"><h3>${tx("Live control","Ĺ˝ivĂ© ovlĂˇdĂˇnĂ­")} ${info("These controls affect production immediately.","Tyto ovlĂˇdacĂ­ prvky mÄ›nĂ­ ĹľivĂ˝ provoz okamĹľitÄ›.")}</h3><div class="button-row"><button class="button ${screen.enabled?"danger":"success"}" data-action="screen.${screen.enabled?"stop":"start"}" data-screen-id="${screen.id}">${screen.enabled?"Stop":"Start"}</button><button class="button ghost" data-action="screen.resync" data-screen-id="${screen.id}">Resync</button><button class="button primary" data-action="playback.return" data-screen-id="${screen.id}">Return Auto</button></div></section>`;decorateHelp($("#inspectorContent"));bindActions($("#inspectorContent"));enhancePickers($("#inspectorContent"));}

  function inspectMedia(media){if(!media)return;$("#inspectorTitle").textContent=media.name;$("#inspectorContent").innerHTML=`${media.thumbnail?`<div class="preview"><img src="${esc(media.thumbnail)}" alt=""></div>`:""}<section class="inspector-section"><h3>${tx("Media details","Detail mĂ©dia")} ${info("Metadata is read from the local file and never exposes unrestricted filesystem paths.","Metadata se ÄŤtou z lokĂˇlnĂ­ho souboru bez odhalenĂ­ neomezenĂ˝ch cest.")}</h3><div class="property"><span>Type</span><b>${esc(media.type)}</b></div><div class="property"><span>Resolution</span><b>${media.width}x${media.height}</b></div><div class="property"><span>Size</span><b>${bytes(media.size)}</b></div><div class="property"><span>Validation</span><b>${media.valid?"ready":esc(media.problem)}</b></div><div class="property"><span>Used by</span><b>${esc(media.references.join(", ")||"none")}</b></div></section><section class="inspector-section"><h3>${tx("Preview and program","NĂˇhled a vysĂ­lĂˇnĂ­")} ${info("Cue prepares the item; Play Now interrupts the selected screen or group after confirmation.","PĹ™ipravit vloĹľĂ­ poloĹľku do fronty; PĹ™ehrĂˇt nynĂ­ po potvrzenĂ­ pĹ™eruĹˇĂ­ plĂˇtno nebo skupinu.")}</h3><div class="field"><label>${tx("Target","CĂ­l")}</label><select id="mediaTarget">${targetOptions(app.liveTarget)}</select></div><div class="button-row"><button class="button ghost" data-media-queue="${esc(media.id)}">${tx("Queue next","ZaĹ™adit dĂˇl")}</button><button class="button danger" data-media-live="${esc(media.id)}">${tx("Play now","PĹ™ehrĂˇt nynĂ­")}</button></div></section>`;decorateHelp($("#inspectorContent"));bindActions($("#inspectorContent"));enhancePickers($("#inspectorContent"));}

  function boolOptions(value){return `<option value="true" ${value?"selected":""}>${tx("Enabled", "Zapnuto")}</option><option value="false" ${!value?"selected":""}>${tx("Disabled", "Vypnuto")}</option>`;}
  function eventStepEditType(step){const type=String(step?.type||"").toLowerCase();return ["text","countdown","wait","manual","wait-manual"].includes(type)?(type==="wait"||type==="manual"?"wait-manual":type):"media";}
  function eventStepTypeOptions(selected){return [["media",tx("Media from library","Medium z knihovny")],["text",tx("Text card","Textova karta")],["countdown",tx("Countdown","Odpocet")],["wait-manual",tx("Wait / hold","Cekani / hold")]].map(([value,label])=>`<option value="${value}" ${value===selected?"selected":""}>${esc(label)}</option>`).join("");}
  function mediaFormValue(value){const raw=String(value||"");const match=app.state.media.find(item=>item.id===raw||item.name===raw);return match?.id||raw;}
  ...
  function inspectPlaylistItem(){
    ...
    const selectedMedia=mediaFormValue(item.value);
    const itemSeconds=Math.max(1,Math.round((item.duration||30000)/1000));
    $("#inspectorContent").innerHTML=`<section class="inspector-section"><h3>${tx("Edit item","Upravit polozku")} ${info("These fields save this playlist item directly. Use Stage only for advanced condition rules.","Tahle pole ulozi polozku primo. Stage pouzij jen pro pokrocila pravidla podminek.")}</h3><div class="field"><label>${tx("Media","Medium")}</label><select id="playlistEditMedia">${mediaOptions(selectedMedia)}</select></div><div class="field-row"><div class="field"><label>${tx("Duration","Delka")}</label><input id="playlistEditDuration" value="${esc(itemSeconds)}s"></div><div class="field"><label>${tx("Weight","Vaha")}</label><input id="playlistEditWeight" type="number" min="1" value="${esc(item.weight)}"></div></div><div class="field"><label>${tx("Enabled","Zapnuto")}</label><select id="playlistEditEnabled">${boolOptions(item.enabled)}</select></div><button class="button primary" data-save-playlist-item="${esc(item.id)}" data-playlist-id="${esc(playlist.id)}">${tx("Save item","Ulozit polozku")}</button></section><section class="inspector-section"><h3>${tx("Selection","Vyber")} ${info("Weight affects probability only after eligibility conditions pass.","Vaha ovlivni pravdepodobnost az po splneni podminek.")}</h3><div class="property"><span>Source</span><b>${esc(item.value)}</b></div><div class="property"><span>Probability</span><b>${fmt(item.probability)}%</b></div><div class="property"><span>Conditions</span><b>${esc(item.conditions)}</b></div>${draftField(`${base}.cooldown`,"duration","Cooldown",`${Math.round(item.cooldown/1000)}s`,"Minimum wait before this item is eligible again.","Minimalni cekani pred dalsim moznym vyberem.")}</section><section class="inspector-section"><h3>${tx("Condition Builder","Editor podminek")} ${info("Each condition is evaluated before cooldown and weighted selection.","Kazda podminka se overi pred cooldownem a vazenym vyberem.")}</h3>${draftField(`${base}.conditions.min-viewers`,"integer",tx("Minimum viewers","Minimum divaku"),0,"Required viewers near this screen.","Pozadovany pocet divaku pobliz platna.")}${draftField(`${base}.conditions.tps-above`,"double",tx("TPS above","TPS vyssi nez"),18,"Minimum safe server TPS.","Minimalni bezpecne TPS serveru.")}${draftField(`${base}.conditions.any-viewer-permission`,"string",tx("Any viewer permission","Opravneni nektereho divaka"),"","At least one nearby viewer must have this permission.","Alespon jeden blizky divak musi mit toto opravneni.")}<div class="field"><label>${tx("Test on screen","Otestovat na platne")} ${info("Evaluates the published conditions against live server state.","Overi publikovane podminky proti zivemu stavu serveru.")}</label><select id="conditionTestScreen">${screenOptions(app.liveTarget)}</select></div><button class="button ghost" data-test-conditions>${tx("Test conditions","Otestovat podminky")}</button></section>`;
    decorateHelp($("#inspectorContent"));bindActions($("#inspectorContent"));enhancePickers($("#inspectorContent"));
  }

  function inspectEventStep(){
    const event=app.state.events.find(item=>item.id===app.selectedEvent),step=event?.steps.find(entry=>entry.id===app.selected.id);
    if(!step)return;
    $("#inspectorTitle").textContent=step.id;
    const base=`events.${event.id}.sequence.${step.id}`;
    const editType=eventStepEditType(step);
    const selectedMedia=editType==="media"?mediaFormValue(step.value):"";
    const textValue=editType==="media"?"":step.value;
    const stepSeconds=Math.max(1,Math.round((step.duration||30000)/1000));
    $("#inspectorContent").innerHTML=`<section class="inspector-section"><h3>${tx("Edit step","Upravit krok")} ${info("This saves the selected event step directly, including its type and value.","Toto ulozi vybrany krok eventu primo, vcetne typu a hodnoty.")}</h3><div class="field"><label>${tx("Step type","Typ kroku")}</label><select id="eventEditStepType">${eventStepTypeOptions(editType)}</select></div><div class="field"><label>${tx("Media value","Hodnota media")}</label><select id="eventEditMedia">${mediaOptions(selectedMedia)}</select></div><div class="field"><label>${tx("Text value","Textova hodnota")}</label><input id="eventEditText" value="${esc(textValue)}" placeholder="${tx("Text, countdown title or wait message","Text, titulek odpoctu nebo cekaci zprava")}"></div><div class="field-row"><div class="field"><label>${tx("Duration","Delka")}</label><input id="eventEditDuration" value="${esc(stepSeconds)}s"></div><div class="field"><label>${tx("Enabled","Zapnuto")}</label><select id="eventEditEnabled">${boolOptions(step.enabled)}</select></div></div><button class="button primary" data-save-event-step="${esc(step.id)}" data-event-id="${esc(event.id)}">${tx("Save step","Ulozit krok")}</button></section><section class="inspector-section"><h3>${tx("Event stage","Krok eventu")} ${info("A stage runs in timeline order unless branch or wait logic changes the flow.","Krok bezi podle osy, pokud tok nezmeni vetev nebo cekani.")}</h3><div class="property"><span>Type</span><b>${esc(step.type)}</b></div><div class="property"><span>Value</span><b>${esc(step.value)}</b></div><div class="property"><span>Conditions</span><b>${esc(step.conditions)}</b></div>${draftField(`${base}.conditions.min-viewers`,"integer",tx("Minimum viewers","Minimum divaku"),0,"The stage waits or skips when too few viewers are nearby.","Krok ceka nebo se preskoci, pokud je pobliz malo divaku.")}${draftField(`${base}.conditions.tps-above`,"double",tx("TPS above","TPS vyssi nez"),18,"Protects the server from expensive stages during low TPS.","Chrani server pred narocnym krokem pri nizkem TPS.")}</section>`;
    decorateHelp($("#inspectorContent"));bindActions($("#inspectorContent"));enhancePickers($("#inspectorContent"));
  }
  function bindView(){
    ...
    $('[data-simulate]')?.addEventListener("click",()=>toast(tx("Probabilities shown are normalized from 1,000 analysis selections.","ZobrazenĂ© pravdÄ›podobnosti jsou normalizovanĂ© z 1 000 analytickĂ˝ch vĂ˝bÄ›rĹŻ.")));
    ...
    $("#newScheduleName")?.addEventListener("input",event=>updateNewAutomationDraft({name:event.target.value}));
    $("#newScheduleTime")?.addEventListener("input",event=>updateNewAutomationDraft({time:event.target.value}));
    $("#newScheduleTime")?.addEventListener("focus",()=>app.deferRenderUntil=Date.now()+10000);
    $("#newScheduleTarget")?.addEventListener("change",event=>updateNewAutomationDraft({target:event.target.value}));
    $("#newScheduleValue")?.addEventListener("change",event=>updateNewAutomationDraft({value:event.target.value}));
    $("#newScheduleAction")?.addEventListener("change",event=>{const value=$("#newScheduleValue");value.innerHTML=automationValueOptions(event.target.value, "");rebuildPicker(value);updateNewAutomationDraft({action:event.target.value,value:value.value || ""});});
    const selectedAutomation = app.selectedAutomation;
    $("#automationEnabled")?.addEventListener("change",event=>updateAutomationDraft(selectedAutomation,{enabled:event.target.value}));
    $("#automationTime")?.addEventListener("input",event=>updateAutomationDraft(selectedAutomation,{time:event.target.value}));
    $("#automationTime")?.addEventListener("focus",()=>app.deferRenderUntil=Date.now()+10000);
    $("#automationTarget")?.addEventListener("change",event=>updateAutomationDraft(selectedAutomation,{target:event.target.value}));
    $("#automationValue")?.addEventListener("change",event=>updateAutomationDraft(selectedAutomation,{value:event.target.value}));
    $("#automationPriority")?.addEventListener("input",event=>updateAutomationDraft(selectedAutomation,{priority:event.target.value}));
    $("#automationConflict")?.addEventListener("change",event=>updateAutomationDraft(selectedAutomation,{conflict:event.target.value}));
    $("#automationAction")?.addEventListener("change",event=>{const value=$("#automationValue");value.innerHTML=automationValueOptions(event.target.value, "");rebuildPicker(value);updateAutomationDraft(selectedAutomation,{action:event.target.value,value:value.value || ""});});
    ...
    $('[data-create-schedule]')?.addEventListener("click",()=>action("schedule.create",{name:$("#newScheduleName").value,time:$("#newScheduleTime").value,target:$("#newScheduleTarget").value,scheduleAction:$("#newScheduleAction").value,value:$("#newScheduleValue").value},false,()=>app.newAutomationDraft=null));
  ...
  function createNamed(actionName,name){if(!name?.trim()){toast(tx("Enter a valid name first.","NejdĹ™Ă­v zadej platnĂ˝ nĂˇzev."),true);return;}action(actionName,{name:name.trim()});}
  ...
  function bindActions(root){
    ...
    root.querySelectorAll('[data-save-playlist-item]').forEach(button=>button.onclick=event=>{event.stopPropagation();action("playlist.item.update",{playlist:button.dataset.playlistId,item:button.dataset.savePlaylistItem,media:$("#playlistEditMedia")?.value || "",duration:$("#playlistEditDuration")?.value || "30s",weight:$("#playlistEditWeight")?.value || "1",enabled:$("#playlistEditEnabled")?.value || "true"});});
    ...
    root.querySelectorAll('[data-save-event-step]').forEach(button=>button.onclick=event=>{event.stopPropagation();action("event.step.update",{event:button.dataset.eventId,step:button.dataset.saveEventStep,stepType:$("#eventEditStepType")?.value || "media",media:$("#eventEditMedia")?.value || "",text:$("#eventEditText")?.value || "",duration:$("#eventEditDuration")?.value || "30s",enabled:$("#eventEditEnabled")?.value || "true"});});
    root.querySelectorAll('[data-save-automation]').forEach(button=>button.onclick=event=>{event.stopPropagation();const id=button.dataset.saveAutomation;action("schedule.update",{schedule:id,time:$("#automationTime")?.value || "20:00",target:$("#automationTarget")?.value || "",scheduleAction:$("#automationAction")?.value || "event",value:$("#automationValue")?.value || "",enabled:$("#automationEnabled")?.value || "true",priority:$("#automationPriority")?.value || "50",conflict:$("#automationConflict")?.value || "priority"},false,()=>delete app.automationDrafts[id]);});
  ...
  async function action(name,data={},confirm=false,onSuccess=null){
    const run=async()=>{try{const result=await request("/api/action",{action:name,...data});toast(result.message);onSuccess?.(result);app.deferRenderUntil=0;await refresh(true);}catch(error){toast(error.message,true);}};
    if(confirm)return ask(tx("Apply live action?","ProvĂ©st Ĺľivou akci?"),tx("This can immediately change what players see.","Toto mĹŻĹľe okamĹľitÄ› zmÄ›nit obsah, kterĂ˝ hrĂˇÄŤi vidĂ­."),run);
    return run();
  }
  ...
  async function publish(){ask(tx("Publish this draft?","Publikovat tento koncept?"),tx("Studio will create a backup, apply staged values and safely reload LuigiScreen.","Studio vytvoĹ™Ă­ zĂˇlohu, pouĹľije pĹ™ipravenĂ© hodnoty a bezpeÄŤnÄ› naÄŤte LuigiScreen."),async()=>{try{const result=await request("/api/publish",{action:"publish"});toast(result.message);await refresh();}catch(error){toast(error.message,true);}});}
  ...
  async function refresh(force=false){if(app.refreshing)return;app.refreshing=true;try{app.state=await request("/api/state");updateChrome();if(force||Date.now()>=app.deferRenderUntil)render(false);}finally{app.refreshing=false;}}
  ...
  function enhancePickers(root=document){
    ...
    root.querySelectorAll("select:not([data-native])").forEach(select=>{
      if(select._luigiChoices || select.closest(".choices"))return;
      select._luigiChoices = new Choices(select,{
        allowHTML:false,
        itemSelectText:"",
        placeholder:true,
        removeItemButton:false,
        searchEnabled:select.options.length>5 || Boolean(select.querySelector("optgroup")),
  ...
  function rebuildPicker(select){
    if(!select)return;
    if(select._luigiChoices){select._luigiChoices.destroy();select._luigiChoices=null;}
    enhancePickers(select.parentElement || document);
  }
  ...
  function fallbackHelp(label){const key=label.toLowerCase();const explanations={state:["Current runtime state reported by the plugin.","AktuĂˇlnĂ­ provoznĂ­ stav hlĂˇĹˇenĂ˝ pluginem."],content:["Media or logical step currently controlling the display.","MĂ©dium nebo krok, kterĂ˝ prĂˇvÄ› Ĺ™Ă­dĂ­ plĂˇtno."],controller:["The playlist, event or direct source responsible for playback.","Playlist, event nebo pĹ™Ă­mĂ˝ zdroj odpovÄ›dnĂ˝ za pĹ™ehrĂˇvĂˇnĂ­."],error:["Last sanitized problem reported by this subsystem.","PoslednĂ­ oÄŤiĹˇtÄ›nĂ˝ problĂ©m hlĂˇĹˇenĂ˝ touto ÄŤĂˇstĂ­."],type:["The typed source or operation category.","Typ zdroje nebo kategorie operace."],resolution:["Pixel width and height detected from the source.","Ĺ Ă­Ĺ™ka a vĂ˝Ĺˇka zdroje v pixelech."],size:["Stored file size on the server.","Velikost souboru uloĹľenĂ©ho na serveru."],validation:["Whether the value can be safely used by LuigiScreen.","Zda mĹŻĹľe LuigiScreen tuto hodnotu bezpeÄŤnÄ› pouĹľĂ­t."],"used by":["Configured playlists and events referencing this item.","Playlisty a eventy, kterĂ© tuto poloĹľku pouĹľĂ­vajĂ­."],probability:["Estimated chance after weights and eligibility rules.","OdhadovanĂˇ Ĺˇance po vyhodnocenĂ­ vah a pravidel."],conditions:["Rules that must pass before this item is eligible.","Pravidla, kterĂˇ musĂ­ poloĹľka splnit pĹ™ed vĂ˝bÄ›rem."],weight:["Relative selection weight among eligible items.","RelativnĂ­ vĂˇha vĂ˝bÄ›ru mezi zpĹŻsobilĂ˝mi poloĹľkami."],duration:["How long this item or stage remains active.","Jak dlouho poloĹľka nebo krok zĹŻstane aktivnĂ­."],cooldown:["Minimum delay before an item may be selected again.","MinimĂˇlnĂ­ prodleva pĹ™ed dalĹˇĂ­m moĹľnĂ˝m vĂ˝bÄ›rem."],enabled:["Whether this object participates in live operation.","Zda se objekt ĂşÄŤastnĂ­ ĹľivĂ©ho provozu."],source:["Media input currently assigned to the screen.","MediĂˇlnĂ­ vstup aktuĂˇlnÄ› pĹ™iĹ™azenĂ˝ plĂˇtnu."],performance:["Rendering speed and frame processing cost.","Rychlost vykreslovĂˇnĂ­ a cena zpracovĂˇnĂ­ snĂ­mkĹŻ."],location:["Stored Minecraft world and block coordinates.","UloĹľenĂ˝ svÄ›t Minecraftu a souĹ™adnice blokĹŻ."],"now playing":["Item players are receiving right now.","PoloĹľka, kterou hrĂˇÄŤi prĂˇvÄ› pĹ™ijĂ­majĂ­."],viewers:["Players currently eligible and close enough to receive this screen.","HrĂˇÄŤi, kteĹ™Ă­ majĂ­ oprĂˇvnÄ›nĂ­ a jsou dostateÄŤnÄ› blĂ­zko plĂˇtnu."],output:["Physical map dimensions of the screen.","FyzickĂ© mapovĂ© rozmÄ›ry plĂˇtna."],"actual fps":["Frames actually rendered after adaptive limits.","SnĂ­mky skuteÄŤnÄ› vykreslenĂ© po adaptivnĂ­m omezenĂ­."]};const pair=explanations[key]||["Explains the value or control shown next to this icon.","VysvÄ›tluje hodnotu nebo ovlĂˇdacĂ­ prvek vedle tĂ©to ikony."];return lang()==="cs"?pair[1]:pair[0];}
  ...
  function buildPalette(){
    ...
    app.state?.screens.forEach(screen=>items.push({label:`${tx("Open screen","OtevĹ™Ă­t plĂˇtno")} ${screen.id}`,detail:"SCREEN",run:()=>{app.view="screens";app.detailScreen=screen.id;app.selected={type:"screen",id:screen.id};render();}}));
    ...
    items.push({label:tx("Return all screens to automation","VrĂˇtit vĹˇechna plĂˇtna automatizaci"),detail:"ACTION",run:()=>app.state.screens.forEach(screen=>action("playback.return",{screen:screen.id}))});
