(() => {
  ...
  function renderScreenAutomation(screen) {
    ...
    const playlistBody = playlists.length ? `<div class="field"><label>${tx("Playlist", "Playlist")} ${info("Assigning a playlist saves it on this screen and starts its weighted rotation immediately.", "Přiřazení playlist uloží na plátno a hned spustí jeho váženou rotaci.")}</label><select id="detailPlaylist">${playlistOptions(selectedPlaylist)}</select></div><div class="button-row"><button class="button primary" data-assign-playlist data-screen-id="${screen.id}">${tx("Assign and play", "Přiřadit a přehrát")}</button><button class="button ghost" data-clear-playlist data-screen-id="${screen.id}" ${screen.playlist ? "" : "disabled"}>${tx("Clear playlist", "Zrušit playlist")}</button></div>` : `<p class="muted">${tx("No playlists exist yet. Create one first, then come back here.", "Zatím neexistuje žádný playlist. Nejdřív ho vytvoř a pak se vrať sem.")}</p><button class="button primary" data-jump="playlists">${tx("Create playlist", "Vytvořit playlist")}</button>`;
    const eventBody = events.length ? `<div class="field"><label>${tx("Event", "Event")} ${info("Events temporarily interrupt the screen, then return it to its playlist or source.", "Event dočasně přeruší plátno a potom ho vrátí na playlist nebo zdroj.")}</label><select id="detailEvent">${eventOptions("")}</select></div><div class="button-row"><button class="button warning" data-start-event-detail data-screen-id="${screen.id}">${tx("Start event", "Spustit event")}</button><button class="button ghost" data-stop-event-detail data-screen-id="${screen.id}">${tx("Stop event", "Zastavit event")}</button></div>` : `<p class="muted">${tx("No events exist yet. Events are optional; playlists are enough for normal playback.", "Zatím neexistují eventy. Eventy jsou volitelné; pro normální přehrávání stačí playlist.")}</p><button class="button ghost" data-jump="events">${tx("Open events", "Otevřít eventy")}</button>`;
    return `<div class="grid-two screen-tab-content"><section class="panel nested-panel">${panelHead(tx("Playlist playback", "Přehrávání playlistu"), "This is the main control for making a playlist play on this screen.", "Toto je hlavní ovládání pro spuštění playlistu na tomhle plátně.", `<span class="tag">${screen.playlist ? esc(screen.playlist) : tx("none", "žádný")}</span>`)}${playlistBody}</section><section class="panel nested-panel">${panelHead(tx("Event override", "Event override"), "Use events for temporary scenes, announcements or manual interruptions.", "Eventy použij pro dočasné scény, oznámení nebo manuální přerušení.", `<span class="tag ${screen.mode === "event" ? "event" : ""}">${esc(screen.mode)}</span>`)}${eventBody}</section></div><div class="inspector-section"><h3>${tx("How this works", "Jak to funguje")}</h3><p class="muted">${tx("A playlist is the normal automatic loop for a screen. An event is temporary and returns to the playlist when it ends. Return auto cancels manual media and lets the playlist/source take over again.", "Playlist je normální automatická smyčka plátna. Event je dočasný a po skončení se vrátí na playlist. Vrátit automatiku zruší manuální média a nechá playlist nebo zdroj znovu převzít řízení.")}</p></div>`;
  }
