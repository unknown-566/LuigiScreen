(() => {
  ...
  function inspectPlaylistItem(){
    ...
    $("#inspectorContent").innerHTML=`<section class="inspector-section"><h3>${tx("Selection","VĂ˝bÄ›r")} ${info("Weight affects probability only after eligibility conditions pass.","VĂˇha ovlivnĂ­ pravdÄ›podobnost aĹľ po splnÄ›nĂ­ podmĂ­nek.")}</h3><div class="property"><span>Probability</span><b>${fmt(item.probability)}%</b></div><div class="property"><span>Conditions</span><b>${esc(item.conditions)}</b></div>${draftField(`${base}.weight`,"integer",tx("Weight","VĂˇha"),item.weight,"Relative weighted chance among eligible items.","RelativnĂ­ vĂˇĹľenĂˇ Ĺˇance mezi zpĹŻsobilĂ˝mi poloĹľkami.")}${draftField(`${base}.duration`,"duration",tx("Duration","DĂ©lka"),`${Math.round(item.duration/1000)}s`,"How long this item controls the screen.","Jak dlouho poloĹľka Ĺ™Ă­dĂ­ plĂˇtno.")}${draftField(`${base}.cooldown`,"duration","Cooldown",`${Math.round(item.cooldown/1000)}s`,"Minimum wait before this item is eligible again.","MinimĂˇlnĂ­ ÄŤekĂˇnĂ­ pĹ™ed dalĹˇĂ­m moĹľnĂ˝m vĂ˝bÄ›rem.")}${draftToggle(`${base}.enabled`,tx("Enabled","Zapnuto"),item.enabled,"Disabled items are excluded before selection.","VypnutĂ© poloĹľky se pĹ™ed vĂ˝bÄ›rem vyĹ™adĂ­.")}</section><section class="inspector-section"><h3>${tx("Condition Builder","Editor podmĂ­nek")} ${info("Each condition is evaluated before cooldown and weighted selection.","KaĹľdĂˇ podmĂ­nka se ovÄ›Ĺ™Ă­ pĹ™ed cooldownem a vĂˇĹľenĂ˝m vĂ˝bÄ›rem.")}</h3>${draftField(`${base}.conditions.min-viewers`,"integer",tx("Minimum viewers","Minimum divĂˇkĹŻ"),0,"Required viewers near this screen.","PoĹľadovanĂ˝ poÄŤet divĂˇkĹŻ poblĂ­Ĺľ plĂˇtna.")}${draftField(`${base}.conditions.tps-above`,"double",tx("TPS above","TPS vyĹˇĹˇĂ­ neĹľ"),18,"Minimum safe server TPS.","MinimĂˇlnĂ­ bezpeÄŤnĂ© TPS serveru.")}${draftField(`${base}.conditions.any-viewer-permission`,"string",tx("Any viewer permission","OprĂˇvnÄ›nĂ­ nÄ›kterĂ©ho divĂˇka"),"","At least one nearby viewer must have this permission.","AlespoĹ jeden blĂ­zkĂ˝ divĂˇk musĂ­ mĂ­t toto oprĂˇvnÄ›nĂ­.")}<div class="field"><label>${tx("Test on screen","Otestovat na plĂˇtnÄ›")} ${info("Evaluates the published conditions against live server state.","OvÄ›Ĺ™Ă­ publikovanĂ© podmĂ­nky proti ĹľivĂ©mu stavu serveru.")}</label><select id="conditionTestScreen">${screenOptions(app.liveTarget)}</select></div><button class="button ghost" data-test-conditions>${tx("Test conditions","Otestovat podmĂ­nky")}</button></section>`;
    decorateHelp($("#inspectorContent"));bindActions($("#inspectorContent"));enhancePickers($("#inspectorContent"));
  }

  function inspectEventStep(){const event=app.state.events.find(item=>item.id===app.selectedEvent),step=event?.steps.find(entry=>entry.id===app.selected.id);if(!step)return;$("#inspectorTitle").textContent=step.id;const base=`events.${event.id}.sequence.${step.id}`;$("#inspectorContent").innerHTML=`<section class="inspector-section"><h3>${tx("Event stage","Krok eventu")} ${info("A stage runs in timeline order unless branch or wait logic changes the flow.","Krok bÄ›ĹľĂ­ podle osy, pokud tok nezmÄ›nĂ­ vÄ›tev nebo ÄŤekĂˇnĂ­.")}</h3><div class="property"><span>Type</span><b>${esc(step.type)}</b></div><div class="property"><span>Value</span><b>${esc(step.value)}</b></div><div class="property"><span>Conditions</span><b>${esc(step.conditions)}</b></div>${draftField(`${base}.duration`,"duration",tx("Duration","DĂ©lka"),`${Math.round(step.duration/1000)}s`,"Maximum or fixed stage duration depending on its type.","MaximĂˇlnĂ­ nebo pevnĂˇ dĂ©lka podle typu kroku.")}${draftToggle(`${base}.enabled`,tx("Enabled","Zapnuto"),step.enabled,"Disabled stages are skipped safely.","VypnutĂ© kroky se bezpeÄŤnÄ› pĹ™eskoÄŤĂ­.")}${draftField(`${base}.conditions.min-viewers`,"integer",tx("Minimum viewers","Minimum divĂˇkĹŻ"),0,"The stage waits or skips when too few viewers are nearby.","Krok ÄŤekĂˇ nebo se pĹ™eskoÄŤĂ­, pokud je poblĂ­Ĺľ mĂˇlo divĂˇkĹŻ.")}${draftField(`${base}.conditions.tps-above`,"double",tx("TPS above","TPS vyĹˇĹˇĂ­ neĹľ"),18,"Protects the server from expensive stages during low TPS.","ChrĂˇnĂ­ server pĹ™ed nĂˇroÄŤnĂ˝m krokem pĹ™i nĂ­zkĂ©m TPS.")}</section>`;decorateHelp($("#inspectorContent"));bindActions($("#inspectorContent"));enhancePickers($("#inspectorContent"));}
